...
/**
 * `/dashboard` route — full-width desktop layout.
 * Port of prototype lines 546–977.
 *
 * Layout: [Sidebar 236px] | [Main column: Topbar + Page] | [ChatRail 372px]
 *
 * Store: `defaultChatStore` is the module singleton shared between this route
 * and the `ChatRail`. Quick-actions and search selections write to surface "d"
 * via `defaultChatStore.getState().send("d", ...)` — the rail reads from the
 * same store instance so messages appear immediately.
 *
 * Below `lg`, the sidebar is hidden and the layout stacks vertically.
 */
export default function DashboardPage() {
  const [dPage, setDPage] = useState<DashboardPage>("overview")

  // ── Quick action handler ────────────────────────────────────────────────────
  // Sends to surface "d" of the shared store so the ChatRail receives it.
