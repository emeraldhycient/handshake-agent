...
- **Hooks** → `hooks/`. **Constants** → `constants/<feature>.ts` (JSX-bearing column configs stay in the section file). **Types** → `types/<feature>.ts` + the `types/index.ts` barrel; import from `@/types`. Per-feature files mirror the `components/<feature>/` directories (`shared`, `chat-cards`, `chat-thread`, `chat-overlays`, `desktop`, `mobile`, `pwa`, `i18n`, …); the legacy `types/components.ts` catch-all is **gone** — never reintroduce it.
