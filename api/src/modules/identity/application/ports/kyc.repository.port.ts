/**
 * DI token and port for the KYC repository.
 *
 * Holds the admin KYC-review writes + the Sumsub webhook write paths (the ONLY
 * writers of a KYC status/tier transition once a Sumsub review posts). All reads
 * go through IIdentityRepository. The legacy synchronous
 * `completeVerification*Atomic` writes were removed with the `/kyc/complete` +
 * `/kyc/submit` endpoints.
 *
