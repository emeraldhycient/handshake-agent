 ...
 *
 * The legacy synchronous NIN/BVN endpoints (`POST /kyc/complete` +
 * `POST /kyc/submit`) were retired along with their HandoffTokenService /
 * WalletService wiring — the controller now takes only (KycService,
 * PinSetupService) and exposes just `setPin` + `createSumsubToken`.
 *
 * All dependencies are mocked. The ThrottlerGuard is bypassed in unit tests.
 */
