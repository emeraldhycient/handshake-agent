 ...
 *
 * READ-ONLY: it never creates, revokes, or mutates Session rows. Selects only
 * non-secret session metadata joined to the bound device for UA/IP telemetry —
 * `accessTokenHash` / `refreshTokenHash` are NEVER selected, so token hashes
 * cannot leak past this port (§3.4 spirit: minimal, non-secret projection).
 *
