...
export const envSchema = z
  ...
  .superRefine((env, ctx) => {
    // Fail-fast boot guards: cross-field invariants that a single field cannot
    // express. Each one closes a credential-disclosure or fail-closed-at-runtime
    // hole, so an invalid combination must throw at boot, not at first use (§7).

    // 1. AUTH_DEV_EXPOSE_OTP echoes OTP / email-verification tokens in HTTP
    //    responses — a credential-disclosure oracle. It must stay off in prod.
    if (env.NODE_ENV === 'production' && env.AUTH_DEV_EXPOSE_OTP === 'true') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['AUTH_DEV_EXPOSE_OTP'],
        message:
          'AUTH_DEV_EXPOSE_OTP must not be "true" when NODE_ENV=production (it exposes OTP/verification tokens in responses).',
      });
    }

    // 2. With real deposits (WALLET_MOCK_MODE=false) the settlement kernel signs
    //    every receipt; a blank RECEIPT_SIGNING_KEY would fail-closed and silently
    //    drop each deposit. Require it up front.
    if (env.WALLET_MOCK_MODE === 'false' && env.RECEIPT_SIGNING_KEY === '') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['RECEIPT_SIGNING_KEY'],
        message:
          'RECEIPT_SIGNING_KEY must be non-empty when WALLET_MOCK_MODE=false (real deposits fail-closed at settle without it).',
      });
    }

    // 3. Statement download links are HMAC-signed; in production a blank
    //    STATEMENT_SIGNING_KEY makes every PDF statement download 503.
    if (env.NODE_ENV === 'production' && env.STATEMENT_SIGNING_KEY === '') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['STATEMENT_SIGNING_KEY'],
        message:
          'STATEMENT_SIGNING_KEY must be non-empty when NODE_ENV=production (statement downloads 503 without it).',
      });
    }

    // 4. With real KYC (KYC_MOCK_MODE=false) NIN/BVN identifiers are stored
    //    encrypted; a blank KYC_ENCRYPTION_KEY would persist them in plaintext.
    if (env.KYC_MOCK_MODE === 'false' && env.KYC_ENCRYPTION_KEY === '') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
