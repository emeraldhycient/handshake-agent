...
- **Hooks** → `hooks/` is the target home, but the promotion is **still pending**: hooks currently live in `lib/hooks/` (there is no `hooks/` directory yet — move them in the admin componentisation wave, don't add new ones to `lib/hooks/`). **Constants** → `constants/<feature>.ts`. **Types** → per-feature `types/<feature>.ts` + the `types/index.ts` barrel; import from `@/types` (never a per-feature file directly, and never the retired `types/components.ts`).
