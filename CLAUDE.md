...
- **What CI actually enforces:** lint, typecheck, `depcruise`, build, the unit suites **with coverage thresholds** (`turbo run test:cov` — api enforces a per-package `coverageThreshold` on the money-path `domain`+`application` globs, so a coverage regression fails CI), **and** the api e2e/testcontainers lane (`~80 supertest suites` in a separate `e2e` job, run `--maxWorkers=2 --workerIdleMemoryLimit=1500M` under a raised `NODE_OPTIONS=--max-old-space-size` so the lane does not OOM). Still run the e2e suites locally before merging anything on the money path; a change is not done until unit + relevant e2e suites are green. **Known CI load-flake:** `whatsapp-voice.e2e-spec.ts` occasionally fails under full-suite parallelism (passes in isolation and in most runs) — re-run the lane rather than treating it as a real regression.
